...
export function CheckInWaitingState({ requestId, onStatusChange }: CheckInWaitingStateProps) {
  ...
  useEffect(() => {
    ...
    eventSource.onerror = () => {
      // EventSource automatically reconnects, but if we need custom error handling, it goes here
  ...
  if (status === "rejected") {
    return (
      <div className="rounded-3xl bg-gradient-to-br from-destructive/15 to-destructive/5 border border-destructive/30 p-10 flex flex-col items-center text-center gap-4 shadow-sm animate-in zoom-in-95 duration-300">
        ...
        <div>
          <h2 className="text-2xl font-bold text-destructive">Check-In Rejected</h2>
  ...
  return (
    <div className="rounded-3xl bg-gradient-to-br from-[#FF9F0A]/15 to-[#FF9F0A]/5 border border-[#FF9F0A]/30 p-10 flex flex-col items-center text-center gap-4 shadow-sm animate-in fade-in duration-300">
      <div className="relative w-20 h-20 flex items-center justify-center">
        <div className="absolute inset-0 rounded-full border-4 border-[#FF9F0A]/20"></div>
        <div className="absolute inset-0 rounded-full border-4 border-[#FF9F0A] border-t-transparent animate-spin"></div>
