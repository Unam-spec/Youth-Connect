...
export default function Dashboard() {
  ...
  useEffect(() => { if (showPinDialog) setPin(""); }, [showPinDialog]);
  ...
  const canViewMembers = session.can_view_members || session.role === "super_admin";
      ...
      <div className="space-y-5 pb-12">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between relative z-10">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold text-teal-400 uppercase tracking-widest">Live</span>
              </div>
        ...
        {session.can_view_kpis && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <KpiCard title="Total Members" icon={<Users className="h-4 w-4 text-teal-400" />} value={kpis?.total_members && kpis.total_members > 0 ? kpis.total_members : undefined} loading={isKpisLoading} lastUpdated={kpisUpdatedAt} accent="teal" />
            <KpiCard title="Today's Attendance" icon={<CheckCircle className="h-4 w-4 text-cyan-400" />} value={kpis?.today_attendance} loading={isKpisLoading} lastUpdated={kpisUpdatedAt} accent="cyan" />
            <KpiCard title="New Visitors" icon={<UserPlus className="h-4 w-4 text-blue-400" />} value={kpis?.today_new_visitors} loading={isKpisLoading} lastUpdated={kpisUpdatedAt} accent="blue" />
            <KpiCard title="Upcoming Events" icon={<Calendar className="h-4 w-4 text-indigo-400" />} value={kpis?.upcoming_events_count} loading={isKpisLoading} lastUpdated={kpisUpdatedAt} accent="indigo" />
          </div>
        )}
          ...
          <div className="rounded-xl border border-teal-500/20 bg-gradient-to-r from-teal-500/8 to-transparent p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-teal-300">Today's Check-ins</span>
              </div>
            ...
            {isAttendanceLoading ? (
              ...
              <div className="flex flex-wrap gap-1.5">
                {attendance.slice(0, 10).map((record: any, i: number) => (
                  <span key={record.id ?? i} className="text-xs px-2.5 py-0.5 rounded-full bg-teal-500/12 text-teal-300 font-medium border border-teal-500/20">
        ...
        <Tabs
          ...
          onValueChange={(val) => { if (val === "leader-pins") fetchLeaderPins(); }}
        >
          <TabsList className="flex flex-wrap h-auto gap-1 justify-start bg-card/60 border border-border/60 p-1.5 rounded-xl backdrop-blur-sm">
            {session.can_view_attendance && (
              <TabsTrigger id="tab-today" value="attendance" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">
                Today
              </TabsTrigger>
            )}
            <TabsTrigger id="tab-checkins" value="checkin-approvals" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">
              Check-ins
            ...
            {canViewMembers && (
              <TabsTrigger id="tab-members" value="members" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">
                Members
              </TabsTrigger>
            )}
            <TabsTrigger id="tab-events" value="events" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">
              Events
            </TabsTrigger>
            <TabsTrigger id="tab-requests" value="requests" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">
              Requests
              {totalRequestBadge > 0 && (
            ...
            {(session.role === "leader" || session.role === "super_admin") && (
              <TabsTrigger id="tab-rsvps" value="rsvps" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">RSVPs</TabsTrigger>
            )}
            {session.role === "super_admin" && (
              <TabsTrigger id="tab-leaders" value="leaders" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">Leaders</TabsTrigger>
            )}
            {session.role === "super_admin" && (
              <TabsTrigger id="tab-leader-pins" value="leader-pins" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">Leader PINs</TabsTrigger>
            )}
            {session.role === "super_admin" && (
              <TabsTrigger id="tab-super-admin" value="super-admin-slots" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">Admin Slots</TabsTrigger>
            )}
            {(session.role === "leader" || session.role === "super_admin") && (
              <TabsTrigger id="tab-channel" value="channel" className="rounded-lg text-xs data-[state=active]:bg-teal-500 data-[state=active]:text-white data-[state=active]:shadow-none">Channel</TabsTrigger>
            )}
          </TabsList>
          ...
          <TabsContent value="attendance" className="mt-4">
            <DashCard>
              <SectionTitle title="Today's Check-ins" icon={<CheckCircle className="h-4 w-4 text-teal-400" />} />
              {isAttendanceLoading ? (
                <SkeletonRows />
              ) : attendance && attendance.length > 0 ? (
                <SimpleTable
          ...
          <TabsContent value="checkin-approvals" className="mt-4">
            <DashCard>
              <div className="flex items-center justify-between mb-1">
                <SectionTitle title="Member Check-in Approvals" icon={<UserCheck className="h-4 w-4 text-teal-400" />} />
                <Button id="btn-refresh-checkins" variant="ghost" size="sm" onClick={fetchPendingCheckIns} disabled={isPendingLoading} className="text-muted-foreground hover:text-teal-400 -mt-4">
                  <RefreshCw className={`w-3.5 h-3.5 mr-1 ${isPendingLoading ? "animate-spin" : ""}`} /> Refresh
                </Button>
              </div>
              <p className="text-xs text-muted-foreground mb-4">
                Only shows existing members who scanned the QR and requested check-in. Auto-refreshes every 30s.
              </p>
              {isPendingLoading && pendingMemberCheckIns.length === 0 ? (
                ...
                <div className="space-y-2">
                  {pendingMemberCheckIns.map(req => (
                    <CheckInCard key={req.id} req={req} onApprove={() => handleApproveCheckIn(req.id)} onReject={() => handleRejectCheckIn(req.id)} />
                  ))}
                </div>
                ...
                {session.role === "super_admin" && (
                  <div className="mb-5 flex items-center justify-between rounded-xl bg-purple-500/10 border border-purple-500/20 px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-purple-300">Super Admin Slots: {superAdminCount} / 4</span>
                    </div>
                    <span className="text-xs text-purple-400 font-medium">Max 4 allowed</span>
                  </div>
                )}
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <SectionTitle title="Member Directory" icon={<Users className="h-4 w-4 text-teal-400" />} />
                  <Input
                    id="member-search"
                    value={search}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 text-sm font-bold ${
                            profile.role === "super_admin" ? "bg-purple-500/20 text-purple-300" :
                            profile.role === "leader" ? "bg-blue-500/20 text-blue-300" :
                            profile.role === "member" ? "bg-teal-500/20 text-teal-300" :
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">{profile.phone || "No phone"}</p>
                        ...
                        <div className="flex items-start gap-2 flex-wrap sm:flex-col sm:items-end sm:shrink-0">
                          {profile.role === "visitor" && (
                            <Button id={`btn-promote-${profile.id}`} size="sm" onClick={() => mutateProfileRole("promote", profile.id)} className="bg-teal-500 hover:bg-teal-400 text-white border-0 h-7 text-xs px-3">
                              Make Member
                            </Button>
                          )}
                          {profile.role === "member" && (
                            <div className="flex items-center gap-2 flex-wrap sm:flex-col sm:items-end">
                              <Button id={`btn-revoke-${profile.id}`} size="sm" variant="outline" onClick={() => mutateProfileRole("revoke", profile.id)} className="h-7 text-xs px-3">Revoke</Button>
                              {session.role === "super_admin" && (
                                <Button id={`btn-make-leader-${profile.id}`} size="sm" onClick={() => setRoleConfirm({ profile, targetRole: "leader" })} className="bg-blue-500 hover:bg-blue-400 text-white border-0 h-7 text-xs px-3">
                                  Make Leader
                                </Button>
                              )}
                            </div>
                          ...
                          {profile.role === "leader" && session.role === "super_admin" && (
                            <div className="flex flex-col gap-2 w-full sm:w-56">
                              <div className="rounded-lg border border-blue-500/20 bg-blue-500/8 p-3 space-y-2">
                                <p className="text-xs font-semibold text-blue-300 mb-1.5">Permissions</p>
                                {[
                                  ...
                                  <div key={key} className="flex items-center justify-between gap-3">
                                    <Label htmlFor={`${key}-${profile.id}`} className="text-xs text-muted-foreground cursor-pointer select-none">{label}</Label>
                                    <Switch
                                      id={`${key}-${profile.id}`}
                                      checked={!!profile[key]}
                              ...
                              <Button id={`btn-make-super-admin-${profile.id}`} size="sm" onClick={() => setRoleConfirm({ profile, targetRole: "super_admin" })} className="bg-purple-500 hover:bg-purple-400 text-white border-0 h-7 text-xs w-full">
                                Make Super Admin
                              </Button>
                            </div>
                          )}
          ...
          <TabsContent value="events" className="mt-4">
            <DashCard>
              {(session.can_create_events || session.role === "super_admin") && (
                <>
                  <SectionTitle title="Create Event" icon={<Calendar className="h-4 w-4 text-teal-400" />} />
                  <div className="grid gap-3 md:grid-cols-2 mb-8">
                    ...
                    <Input type="number" value={eventForm.age_min} onChange={e => setEventForm(c => ({ ...c, age_min: e.target.value }))} placeholder="Min age (optional)" />
                    <Input type="number" value={eventForm.age_max} onChange={e => setEventForm(c => ({ ...c, age_max: e.target.value }))} placeholder="Max age (optional)" />
                    <Textarea value={eventForm.description} onChange={e => setEventForm(c => ({ ...c, description: e.target.value }))} placeholder="Description (optional)" className="md:col-span-2 resize-none" />
                    <div className="flex items-center gap-3">
                      <Switch id="is-public" checked={eventForm.is_public} onCheckedChange={checked => setEventForm(c => ({ ...c, is_public: checked }))} className="data-[state=checked]:bg-teal-500" />
                      <Label htmlFor="is-public" className="cursor-pointer">Show on landing page</Label>
                    </div>
                    <Button id="btn-create-event" onClick={handleCreateEvent} disabled={createEvent.isPending} className="bg-teal-500 hover:bg-teal-400 text-white border-0">
              ...
              <SectionTitle title="All Events" icon={<Calendar className="h-4 w-4 text-blue-400" />} />
              {isEventsLoading ? <Skeleton className="h-32 w-full rounded-xl" /> : events && events.length > 0 ? (
                <SimpleTable
                  ...
                  rows={events.map((event: any) => [
                    ...
                    event.location,
                    event.is_public
                      ? <span className="text-xs text-teal-400 font-medium">Public</span>
                      : <span className="text-xs text-muted-foreground">Internal</span>,
                    (session.role === "leader" || session.role === "super_admin") ? (
                      <Button id={`btn-delete-event-${event.id}`} variant="ghost" size="sm" onClick={() => { setDeleteEventId(event.id); setDeleteEventName(event.title); }} className="text-destructive hover:text-destructive h-7 w-7 p-0">
          ...
          <TabsContent value="requests" className="mt-4">
            <DashCard>
              ...
              {(isPendingLoading && pendingFirstTimers.length === 0) ? null : pendingFirstTimers.length > 0 ? (
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-1">
                    <SectionTitle title="First-Timer Registrations" icon={<UserPlus className="h-4 w-4 text-amber-400" />} />
                    <Badge className="bg-amber-500/15 text-amber-300 border-amber-500/30 -mt-4" variant="outline">
                      {pendingFirstTimers.length} waiting
                    </Badge>
                  </div>
                  <p className="text-xs text-muted-foreground mb-4">
                    These first-time visitors have registered and are waiting for a leader to approve their attendance.
                  </p>
                  <div className="space-y-2">
                    {pendingFirstTimers.map(req => (
                      <CheckInCard key={req.id} req={req} onApprove={() => handleApproveCheckIn(req.id)} onReject={() => handleRejectCheckIn(req.id)} isFirstTimer />
                    ))}
                  </div>
              ...
              <SectionTitle title="Membership Requests" icon={<UserCheck className="h-4 w-4 text-teal-400" />} />
              <p className="text-xs text-muted-foreground mb-4">Visitors applying to become full members of the church.</p>
              {isRequestsLoading ? (
                <SkeletonRows />
              ) : requests && requests.length > 0 ? (
                <div className="space-y-2">
                  {requests.map((request: any) => (
                    <div key={request.id} className="flex flex-col gap-3 rounded-xl border border-border/50 bg-card/30 p-4 sm:flex-row sm:items-center sm:justify-between hover:border-teal-500/25 transition-colors">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-semibold text-sm">{request.profile?.full_name ?? "Unknown visitor"}</p>
                          <Badge variant="outline" className="text-xs text-amber-300 border-amber-500/30">Membership</Badge>
                        </div>
                        <p className="text-xs text-muted-foreground mt-0.5">{request.reason}</p>
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <Button id={`btn-approve-req-${request.id}`} size="sm" onClick={() => mutateRequest("approve", request.id)} className="bg-teal-500 hover:bg-teal-400 text-white border-0 h-7 text-xs">Approve</Button>
                        <Button id={`btn-reject-req-${request.id}`} size="sm" variant="outline" onClick={() => mutateRequest("reject", request.id)} className="h-7 text-xs">Reject</Button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <p className="text-xs text-muted-foreground py-4 text-center">No membership requests.</p>
              )}
            </DashCard>
          </TabsContent>
          ...
          {(session.role === "leader" || session.role === "super_admin") && (
            <TabsContent value="rsvps" className="mt-4">
              <DashCard>
                <div className="flex items-center justify-between mb-1">
                  <SectionTitle title="Event RSVPs" icon={<Calendar className="h-4 w-4 text-blue-400" />} />
                  <Button id="btn-refresh-rsvps" variant="ghost" size="sm" onClick={fetchRsvps} disabled={isRsvpsLoading} className="text-muted-foreground hover:text-teal-400 -mt-4">
                    <RefreshCw className={`w-3.5 h-3.5 mr-1 ${isRsvpsLoading ? "animate-spin" : ""}`} /> Refresh
                  </Button>
                </div>
                <div className="mb-5">
                  <Label htmlFor="event-select" className="text-xs text-muted-foreground mb-1.5 block">Select Event</Label>
                  <select
                    id="event-select"
                    ...
                    <Card className="border-green-500/20 bg-green-500/5">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm flex items-center justify-between text-green-300">
                          Going <Badge className="bg-green-600/80 text-white border-0">{rsvps.filter((r: any) => r.status === "going").length}</Badge>
                        </CardTitle>
                      </CardHeader>
                    ...
                    <Card className="border-red-500/20 bg-red-500/5">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm flex items-center justify-between text-red-300">
                          Not Going <Badge className="bg-red-600/80 text-white border-0">{rsvps.filter((r: any) => r.status === "not_going").length}</Badge>
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="pt-0">
          ...
          {session.role === "super_admin" && (
            <TabsContent value="leaders" className="mt-4">
              <DashCard>
                <SectionTitle title="Leader Directory" icon={<Shield className="h-4 w-4 text-blue-400" />} />
                {isLeadersLoading ? <SkeletonRows /> : leaders && leaders.length > 0 ? (
                  <div className="space-y-2.5">
                    {leaders.map((leader: any, i: number) => (
                      <div key={leader.profile?.id ?? i} className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
                        <div className="flex items-center gap-3 mb-3">
                          <div className="h-9 w-9 rounded-full bg-blue-500/20 flex items-center justify-center text-sm font-bold text-blue-300 shrink-0">
                            {leader.profile?.full_name?.charAt(0)?.toUpperCase() ?? "L"}
                          </div>
                          <div>
                            <p className="font-semibold text-sm">{leader.profile?.full_name ?? "Unknown"}</p>
                            <p className="text-xs text-muted-foreground capitalize">{leader.profile?.role ?? "leader"}</p>
                          </div>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {[
                            ...
                            <span key={key} className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${
                              leader[key]
                  ...
                  <div className="rounded-xl border border-dashed border-blue-500/25 p-10 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">No delegated leaders yet.</p>
                    <p className="text-xs text-muted-foreground mt-1">Promote a member to Leader from the Members tab.</p>
                  </div>
                )}
              </DashCard>
            </TabsContent>
          ...
          {session.role === "super_admin" && (
            <TabsContent value="leader-pins" className="mt-4">
              <DashCard>
                <div className="flex items-center justify-between mb-1">
                  <SectionTitle title="Leader PINs" icon={<Shield className="h-4 w-4 text-teal-400" />} />
                  <Button id="btn-refresh-pins" variant="ghost" size="sm" onClick={fetchLeaderPins} disabled={isLeaderPinsLoading} className="text-muted-foreground hover:text-teal-400 -mt-4">
                    <RefreshCw className={`w-3.5 h-3.5 mr-1 ${isLeaderPinsLoading ? "animate-spin" : ""}`} /> Refresh
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground mb-5">Visible to super admins only. PINs are used for leader login authentication.</p>
                {isLeaderPinsLoading ? (
                  <SkeletonRows />
                ) : leaderPins.length > 0 ? (
                  <div className="space-y-2">
                    {leaderPins.map(leader => (
                      <div key={leader.id} className="flex items-center justify-between rounded-xl border border-border/50 bg-card/30 p-4 hover:border-teal-500/30 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-sm">{leader.full_name}</p>
                  ...
                  <div className="rounded-xl border border-dashed border-teal-500/20 p-10 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">No leaders with PINs found.</p>
                    <p className="text-xs text-muted-foreground mt-1">Leaders can set their PIN from the Admin Slots tab.</p>
                  </div>
                )}
              </DashCard>
            </TabsContent>
          ...
          {session.role === "super_admin" && (
            <TabsContent value="super-admin-slots" className="mt-4">
              <DashCard>
                <SectionTitle title="Super Admin Slots" icon={<Star className="h-4 w-4 text-purple-400" />} />
                {isProfilesLoading ? <SkeletonRows /> : (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <div className="flex gap-1">
                        {Array.from({ length: 4 }).map((_, i) => (
                          <div key={i} className={`h-2 w-8 rounded-full ${i < superAdminCount ? "bg-purple-500" : "bg-muted"}`} />
                        ))}
                      </div>
                      <span className="text-xs text-muted-foreground">{superAdminCount} of 4 slots filled</span>
                    </div>
                    <div className="space-y-2">
                      {profiles?.filter((p: any) => p.role === "super_admin").map((admin: any) => (
                        <div key={admin.id} className="flex items-center justify-between rounded-xl border border-purple-500/20 bg-purple-500/5 p-4">
                          <div className="flex items-center gap-3">
                          ...
                          <Badge className="bg-purple-500/15 text-purple-300 border-purple-500/30" variant="outline">Super Admin</Badge>
                        </div>
                      ))}
                    </div>
                    <div className="border-t border-border/40 pt-4">
                      ...
                      <p className="text-xs text-muted-foreground mb-3">
                        {hasPin ? "PIN is set — used for leader authentication." : "No PIN set yet. Set one to enable leader login."}
                      </p>
                      <Button id="btn-set-pin" variant="outline" size="sm" onClick={() => setShowPinDialog(true)} className="border-teal-500/30 hover:border-teal-500 hover:text-teal-300 transition-colors">
          ...
          {(session.role === "leader" || session.role === "super_admin") && (
            <TabsContent value="channel" className="mt-4">
              <DashCard>
                <SectionTitle title="Leader Channel" icon={<Users className="h-4 w-4 text-blue-400" />} />
                <div className="rounded-xl border border-dashed border-blue-500/20 p-12 text-center">
                  ...
                  <p className="text-sm font-semibold text-foreground/80 mb-1">Coming Soon</p>
                  <p className="text-xs text-muted-foreground">A private channel for leaders is on its way.</p>
                </div>
              </DashCard>
            </TabsContent>
          )}
      ...
      <Dialog open={showPinDialog} onOpenChange={setShowPinDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button id="btn-save-pin" onClick={handleSavePin} disabled={pin.length !== 4} className="bg-teal-500 hover:bg-teal-400 text-white border-0">
...
function SkeletonRows({ count = 3 }: { count?: number }) {
  return (
    <div className="space-y-2.5">
      {Array.from({ length: count }).map((_, i) => <Skeleton key={i} className="h-14 w-full rounded-xl" />)}
    </div>
  );
}
...
function CheckInCard({
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between transition-colors ${
      isFirstTimer
      ...
      <div className="flex items-start gap-3">
        <div className={`h-9 w-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
          isFirstTimer ? "bg-amber-500/15 text-amber-300" : "bg-teal-500/15 text-teal-300"
        }`}>
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isFirstTimer && (
              <Badge variant="outline" className="text-xs text-amber-300 border-amber-500/30 py-0">First Timer</Badge>
            )}
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">
            {req.phone ?? "No phone"} · {format(new Date(req.requested_at), "HH:mm")}
          </p>
        </div>
      </div>
      <div className="flex gap-2 shrink-0">
...
function RoleBadge({ role }: { role: string }) {
  let classes = "";
  switch (role) {
    case "super_admin": classes = "bg-purple-500/12 text-purple-300 border-purple-500/25"; break;
    case "leader": classes = "bg-blue-500/12 text-blue-300 border-blue-500/25"; break;
    case "member": classes = "bg-teal-500/12 text-teal-300 border-teal-500/25"; break;
    default: classes = "bg-muted/60 text-muted-foreground border-border/50";
  }
  return (
...
function KpiCard({ title, value, loading, icon, lastUpdated, accent }: {
  title: string;
  value?: number;
  loading: boolean;
  icon: ReactNode;
  ...
  return (
    <Card className={`border-t-2 ${borderMap[accent ?? "teal"]} bg-card/60 backdrop-blur-sm`}>
      ...
      <CardContent className="px-4 pb-4">
        {loading ? (
          <><Skeleton className="h-8 w-14" /><Skeleton className="mt-1.5 h-2.5 w-24" /></>
        ) : (
          <>
            <div className="text-3xl font-bold tabular-nums">{value ?? 0}</div>
            {lastUpdated && <p className="mt-1 text-xs text-muted-foreground">Updated {lastUpdated}</p>}
          </>
        )}
      </CardContent>
    </Card>
    ...
    <div className="overflow-x-auto rounded-xl border border-border/50">
      <table className="w-full text-sm">
        <thead className="bg-muted/25 border-b border-border/50">
          <tr>
            {headers.map(h => (
              <th key={h} className="px-4 py-2.5 text-left font-medium text-xs text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-border/40">
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-muted/15 transition-colors">
              {row.map((cell, j) => <td key={`${i}-${j}`} className="px-4 py-3 text-sm">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
