          ...
          {(session.role === "leader" || session.role === "super_admin") && (
            <TabsContent value="channel" className="mt-4">
              <div className="border border-border/50 rounded-2xl bg-card/40 backdrop-blur-sm overflow-hidden flex flex-col h-[650px] shadow-2xl relative">
                ...
                <div className="p-4 border-b border-border/50 flex items-center justify-between bg-muted/10">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-2xs text-muted-foreground mt-0.5">Private channel for leaders &amp; admins</p>
                    </div>
                  </div>

                  {/* Real-time Status Indicator */}
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-950/40 border border-slate-800/80 text-3xs font-semibold tracking-wider uppercase">
                    <span className="relative flex h-2 w-2">
                ...
                >
                  {chatMessages.length > 0 ? (
                    chatMessages.map((msg) => {
                      const isMe = msg.sender_id === session.profile_id;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="space-y-1">
                            ...
                            <div
                              className={`flex items-center gap-1.5 text-3xs text-muted-foreground px-1 ${
                ...
                >
                  <Input
                    placeholder="Type a co-ordination message..."
                    value={chatInput}
                    onChange={(e) => setChatInput(e.target.value)}
                    disabled={isSendingChatMessage}
                    maxLength={1000}
                    className="flex-grow bg-slate-950/60 border-slate-700/80 text-white placeholder:text-slate-500 focus:border-teal-500 focus:ring-teal-500 rounded-xl h-11 px-4 text-sm"
                  />
                  <Button
                    type="submit"
                    disabled={isSendingChatMessage || !chatInput.trim()}
                    className="h-11 w-11 rounded-xl bg-teal-500 hover:bg-teal-400 border-0 flex items-center justify-center p-0 text-white shadow-lg shadow-teal-500/10 active:scale-95 duration-150 shrink-0"
                  >
