...
export async function customFetch<T = unknown>(
  ...
  // Attach leader session header for PIN-based authentication (used by leader dashboard)
  if (!headers.has("x-leader-session") && typeof window !== "undefined") {
    try {
      ...
      if (sessionStr) {
        ...
        if (Date.now() < session.expires_at) {
          headers.set("x-leader-session", sessionStr);
        }
      }
    } catch {
